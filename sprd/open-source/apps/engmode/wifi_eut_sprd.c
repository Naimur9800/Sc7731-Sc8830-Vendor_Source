...
static int start_wifieut(char *rsp)
{
    ...
    ENG_LOG("ADL %s(), callED system(wcnd_cli wcn poweron), ret = %d", __func__, ret);
#endif
...
static int stop_wifieut(char *rsp)
{
	...
	ENG_LOG("ADL %s(), callED system(wcnd_cli wcn poweroff ), ret = %d", __func__, ret);
#endif
...
int wifi_mac_filter_set(int on_off, const char *mac, char *rsp)
{
    ...
    if (1 == on_off)
    {
        ...
        {
            ENG_LOG("%s(), mac's length is invalid, len = %lu", __func__, strlen(mac));
            goto err;
        }
