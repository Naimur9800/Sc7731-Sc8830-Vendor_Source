...
static int do_read(void)
{
	...
	LOGD("mmitestsensor here tpye_num=%d\n",type_num);
	/*********activate_sensors(ON)***************/
	for (i = 0; i < count; i++) {
		err = activate_sensors(i, 1, S_ON);
		if (err != 0) {
			LOGD("mmitestsensor activate_sensors(ON) for '%s'failed \n",list[i].name);
			return 0;
		}
	}
	do {
...
funend:
	...
	if (err != 0) {
		LOGD("mmitestsensor activate_sensors(OFF) for '%s'failed\n",list[i].name);
		return 0;
		}
	}
...
static void msensor_thread(void *param)
{
	...
	ptr=list[1].name;
	LOGD("mmitestsensor here m2 %s",ptr);
	ui_set_color(CL_WHITE);
