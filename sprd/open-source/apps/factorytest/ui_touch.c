...
static void show_event(int fd, struct input_event *ev) {
    ...
    LOGI("touchevent: fd=%d, type=%s, code=%s, val=%d count=%d\n", fd, evtypestr, evcodestr, ev->value,count);
#endif
}
...
void touch_handle_input(int fd, struct input_event *ev) {
	...
	//while(testde)
	//	LOGD("touchevent %s",testde->dname);
    ...
    LOGE("touchevent %d\n",count);
	for (i = 0; i < MAX_INPUT_DEVS; i++) {
	LOGE("touchevent input_devices[%d].fd=%d\n",i,input_devices[i].fd);
	...
	LOGE("touchevent 1touch_min.x=%d\n",dev->touch_min.x);
	LOGE("touchevent 2touch_max.x=%d\n",dev->touch_max.x);
	LOGE("touchevent 3touch_pos.x=%d\n",dev->touch_pos.x);
	LOGE("touchevent 4touch_min.y=%d\n",dev->touch_min.y);
	LOGE("touchevent 5touch_max.y=%d\n",dev->touch_max.y);
	LOGE("touchevent 6touch_pos.y=%d\n",dev->touch_pos.y);
