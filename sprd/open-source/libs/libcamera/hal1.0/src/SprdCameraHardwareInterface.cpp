...
int SprdCameraHardware::Callback_PreviewMalloc(cmr_u32 size, cmr_u32 sum, cmr_uint *phy_addr, cmr_uint *vir_addr)
{
	...
	if (mPreviewDcamAllocBufferCnt>0 && NULL == mPreviewHeapArray) {
		...
		if(0 == strcmp(mParameters.getPreviewFormat(), "yuv420p")){//bug 440213 for cts verifier
			...
			mPreviewHeapArray_vir[buffer_end_id] = (uint32_t)PreviewHeap->data;
		}
...
void SprdCameraHardware::PushAllZslBuffer(void)
{
	...
	for (i = 0; i < mZslHeapNum; i++) {
		camera_set_zsl_buffer(mCameraHandle, (cmr_uint)mZslHeapArray[i]->phys_addr, (cmr_uint)mZslHeapArray[i]->data);
	}
}
...
int SprdCameraHardware::Callback_ZslMalloc(cmr_u32 size, cmr_u32 sum, cmr_uint *phy_addr, cmr_uint *vir_addr)
{
	...
	if (NULL == mZslHeapArray) {
		...
		for (i=buffer_start_id ; i<buffer_end_id ; i++) {
			...
			if (NULL == ZslHeap) {
				LOGE("Callback_ZslMalloc: error PreviewHeap is null, index %ld", i);
				return false;
			}
	...
	if (type == CAMERA_PREVIEW_RESERVED) {
		if(mPreviewHeapReserved == NULL) {
			...
			*vir_addr++ = (cmr_uint)memory->data;
			...
			LOGI("malloc Preview memory, malloced type %d,request num %d, request size 0x%x",
				type, sum, size);
		...
		if(mVideoHeapReserved == NULL) {
			...
			*vir_addr++ = (cmr_uint)memory->data;
			...
			LOGI("malloc Video memory, malloced type %d,request num %d, request size 0x%x",
				type, sum, size);
		...
		if(mZslHeapReserved == NULL) {
			...
			*vir_addr++ = (cmr_uint)memory->data;
			...
			LOGI("malloc ZSL memory, malloced type %d,request num %d, request size 0x%x",
				type, sum, size);
...
camera_frame_type *SprdCameraHardware::popZSLQueue()
{
	List<camera_frame_type>::iterator frame;
	camera_frame_type ret;

	if(mZSLQueue.size() == 0)
		return NULL;

	frame = mZSLQueue.begin()++;
	ret = static_cast<camera_frame_type>(*frame);
...
void SprdCameraHardware::pushZSLQueue(camera_frame_type frame)
{
...
{
	List<camera_frame_type>::iterator round;
...
void SprdCameraHardware::receivePreviewFrame(struct camera_frame_type *frame)
{
	...
	LOGI("receivePreviewFrame lls_info = %d", metadata.light_condition);
	camera_face_t face_info[FACE_DETECT_NUM];
	uint32_t k = 0;
	...
	if (isPreviewing()) {
		...
		if (PREVIEW_ZSL_FRAME == frame->type) {
			if (1 == mZslShotPushFlag) {
				LOGI("receivePreviewFrame getZSLQueueFrameNum %d", getZSLQueueFrameNum());
				for (i=getZSLQueueFrameNum(); i>mZSLFrameNum; i--) {
					LOGI("receivePreviewFrame getZSLQueueFrameNum %d", getZSLQueueFrameNum());
					zsl_frame = popZSLQueue();
					if (!need_pause) {
						camera_set_zsl_buffer(mCameraHandle, zsl_frame->y_phy_addr, zsl_frame->y_vir_addr);
					}
				}
				if (0 == getZSLQueueFrameNum()) {
					Mutex::Autolock zsllock(&mZslBufLock);
					zsl_frame = frame;
					camera_set_zsl_snapshot_buffer(mCameraHandle,zsl_frame->y_phy_addr, zsl_frame->y_vir_addr);
					mZslChannelStatus = 0;
				} else {
					Mutex::Autolock zsllock(&mZslBufLock);
					zsl_frame = popZSLQueue();
				...
				if (SPRD_INTERNAL_RAW_REQUESTED == getCaptureState() && need_pause) {
					pushZSLQueue(*frame);
			...
			if (!isCapturing() || !need_pause || mZslChannelStatus) {
				pushZSLQueue(*frame);
				if (mZSLFrameNum < getZSLQueueFrameNum()) {
					zsl_frame = popZSLQueue();
					camera_set_zsl_buffer(mCameraHandle, zsl_frame->y_phy_addr, zsl_frame->y_vir_addr);
			...
			if (!isCapturing() || !need_pause || mZslChannelStatus) {
				camera_set_zsl_buffer(mCameraHandle, frame->y_phy_addr, frame->y_vir_addr);
			}
		}
	} else {
		...
		if (mData_cb) {
			...
			if (1 == mCaptureNum) {
				if (mMsgEnabled & CAMERA_MSG_COMPRESSED_IMAGE){
					...
					memcpy((void *)((uint32_t)(mem->data) + frame_size), (void*)(frame->uv_vir_addr), (frame_size >> 1));
