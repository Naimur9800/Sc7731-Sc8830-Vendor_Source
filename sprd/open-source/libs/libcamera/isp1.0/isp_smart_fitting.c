...
static int32_t _isp_denoise_get_region(struct isp_smart_in_param *in_param_ptr,struct region_info *region_ptr);
static uint32_t _isp_smart_linear_interpolate(struct isp_smart_scene *scene, int32_t value);
static int32_t _isp_gamma_get_region(struct isp_smart_in_param *in_param_ptr,struct region_info *region_ptr);
...
int32_t isp_smart_get_level(struct isp_smart_in_param *in_param_ptr, uint32_t *level)
{
	...
	if(ISP_INPUT_ERROR == _isp_denoise_get_region(in_param_ptr,&region)){
		*level = 0;
		goto GET_LEVEL_EXIT;
	}
	...
	case TRANS_REGION:
		...
		if(bv[scene1-1].bv_high != bv[scene1-1].bv_low){
			*level=(uint32_t)(((bv[scene1-1].bv_high-cur_bv)*level2 +(cur_bv-bv[scene1-1].bv_low)*level1)*1.0/(bv[scene1-1].bv_high-bv[scene1-1].bv_low)+0.5);
		}else{
...
int32_t isp_smart_get_gamma_index(struct isp_smart_in_param *in_param_ptr, uint32_t *index1, uint32_t *index2, uint32_t *bv_index)
{
	...
	if(ISP_INPUT_ERROR == _isp_gamma_get_region(in_param_ptr,&region)){
		*index1 = 0;
		*index2 = 0;
		*bv_index = 0;
		goto GET_INDEX_EXIT;
...
int32_t isp_smart_get_index(struct isp_smart_in_param *in_param_ptr, uint32_t *index)
{
	...
	case TRANS_REGION:
		...
		if(bv[scene1].bv_high != bv[scene1].bv_low){
			*index = (uint32_t)(((bv[scene1].bv_high-cur_bv)*index2 +(cur_bv-bv[scene1].bv_low)*index1)*1.0/(bv[scene1].bv_high-bv[scene1].bv_low)+0.5);
		}else{
