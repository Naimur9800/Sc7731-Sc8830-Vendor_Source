...
static int32_t  _calc_piecewise_func0(struct isp_smart_interplate_piecewise_func *func, int32_t  x, int32_t value[2], uint32_t weight[2])
{
...
int32_t isp_smart_calculation(isp_u32 func_type,
						struct isp_smart_interplate_piecewise_func* cur_func,
								void* smart_cur_info_in,
								void* smart_calc_param_out)
{
...
*/
	...
	switch (inter_func_type) {
		case ISP_SMART_INTERPLATE_FUNC0:
		{
			...
			ISP_LOGI("ISP_SMART: func=%d, input=%d, output=(%d, %d), (%d, %d)", inter_func_type, input_ptr->x, output_ptr->index[0],
								output_ptr->index[1], output_ptr->weight[0], output_ptr->weight[1]);
		}
		break;
