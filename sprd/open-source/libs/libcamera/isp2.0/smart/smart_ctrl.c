...
static int32_t _smart_set_workmode(struct smart_context *cxt, void *in_param)
{
...
static int32_t _smart_set_flash(struct smart_context *cxt, void *in_param)
{
	int32_t rtn = ISP_SUCCESS;
...
static int32_t _check_handle_validate(smart_handle_t handle)
 {
...
static int32_t _check_block_param(struct isp_smart_block_cfg *blk_cfg)
{
	int32_t rtn = ISP_SUCCESS;
	...
	for (i=0; i<blk_cfg->component_num; i++) {
			...
			for (k=0; k<comp_cfg->section_num; k++) {
				...
				if ((k < comp_cfg->section_num - 1) &&
					(bv_range->max > comp_cfg->bv_range[k+1].min)) {
...
static int32_t _check_param(struct isp_smart_param *param)
{
	int32_t rtn = ISP_SUCCESS;
	uint32_t i = 0;
		...
		for (j=0; j<blk_cfg->component_num; j++) {
			...
			for (k=0; k<comp_cfg->section_num; k++) {
				...
				ISP_LOGI("  section[%d], bv=[%d, %d], func num=%d",
						k, comp_cfg->bv_range[k].min,
						comp_cfg->bv_range[k].max, func->num);

				if (bv_range->min > bv_range->max) {
					ISP_LOGI("  section[%d]: bv_range is invalid.\n",k);
				}

				if ((k+1 < comp_cfg->section_num) &&
					(bv_range->max > comp_cfg->bv_range[k+1].min)) {
					ISP_LOGI("  section[%d]: bv_range.max=%d, section[%d]: bv_range.min=%d, bv_range is invalid.",
								k, bv_range->max, k+1, comp_cfg->bv_range[k+1].min);
				}
			...
			if (0 == comp_cfg->section_num) {
				...
				for (m=0; m<func->num; m++) {
					...
					ISP_LOGI("    [%d]=(%4d, %4d)",
						m, func->samples[m].x, func->samples[m].y);
				}
			}
		}
	}
...
static int32_t _parse_tuning_param(struct smart_tuning_param src[], struct tuning_param dst[], uint32_t num)
{
	int32_t rtn = ISP_SUCCESS;
	uint32_t i = 0;
	...
	for (i=0; i<num; i++) {
		if (NULL != src[i].data.data_ptr && src[i].data.size > 0) {
			if (0 == src[i].version) {
				...
				rtn = _check_param((struct isp_smart_param *)src[i].data.data_ptr);
...
static int32_t _smart_get_update_param(struct smart_context *cxt, void *in_param)
{
	int32_t rtn = ISP_SUCCESS;
	uint32_t i = 0;
	...
	rtn = _parse_tuning_param(param->tuning_param, cxt->tuning_param, SMART_MAX_WORK_MODE);
	if (ISP_SUCCESS != rtn) {
		ISP_LOGE("_parse_tuning_param failed: rtn = %d", rtn);
		goto ERROR_EXIT;
	}
...
static int32_t _piecewise_func_v1(struct isp_piecewise_func *func, int32_t  x, 
...
static int32_t _piecewise_func_v0(struct isp_piecewise_func *func, int32_t x, int32_t *result)
{
	int32_t rtn = ISP_SUCCESS;
	uint32_t num = func->num;
	struct isp_sample *samples = func->samples;
...
static int32_t _calc_func(struct isp_piecewise_func *func, uint32_t y_type, isp_s32 x, struct isp_weight_value *result)
{
	int32_t rtn = ISP_SUCCESS;
...
static int32_t _calc_bv_section(struct isp_range bv_range[], uint32_t num,
	 			int32_t bv, struct isp_weight_value *result)
{
	int32_t rtn = ISP_SUCCESS;
	...
	if (ISP_SUCCESS == rtn) {
		...
		result->weight[0] = bv_distance[1] * SMART_WEIGHT_UNIT
...
int32_t _calc_component(struct isp_smart_component_cfg *cfg, int32_t bv, int32_t bv_gain, uint32_t ct,
						struct smart_component_result *result)
{
	...
	case ISP_SMART_X_TYPE_BV:
		rtn = _calc_func(&cfg->func[0], cfg->y_type, bv, &func_result);
		break;

	case ISP_SMART_X_TYPE_BV_GAIN:
		rtn = _calc_func(&cfg->func[0], cfg->y_type, bv_gain, &func_result);
		break;

	case ISP_SMART_X_TYPE_CT:
		rtn = _calc_func(&cfg->func[0], cfg->y_type, ct, &func_result);
		break;
	...
	case ISP_SMART_X_TYPE_BV_CT:
		{ 
			...
			rtn = _calc_bv_section(cfg->bv_range, cfg->section_num, bv, &bv_result);
			if (ISP_SUCCESS != rtn) {
				return rtn;
			}
			...
			for (i=0; i<2; i++) {
				...
				if (bv_weight > 0 && bv_idx < cfg->section_num) {
					rtn = _calc_func(&cfg->func[bv_idx], cfg->y_type, ct, &tmp_result[i]);
					if (ISP_SUCCESS != rtn)
						return rtn; 
				}
			}
...
int32_t _calc_component_flash(struct isp_smart_component_cfg *cfg, struct smart_component_result *result)
{
	int32_t rtn = ISP_SUCCESS;
	struct isp_weight_value func_result = {{0}, {0}};
	struct isp_weight_value *fix_data = (struct isp_weight_value *)result->fix_data;
...
int32_t _calc_block(struct isp_smart_block_cfg *cfg, int32_t bv, uint32_t bv_gain, uint32_t ct, struct smart_block_result *result,
			enum smart_ctrl_flash_mode flash_mode)
{
	int32_t rtn = ISP_SUCCESS;
	uint32_t i = 0;
	...
	rtn = _check_block_param(cfg);
	if (ISP_SUCCESS != rtn) {
		ISP_LOGV("ISP_TAG: block param check failed.\n");
		return rtn;
	}

	component_num = component_num > ISP_SMART_MAX_VALUE_NUM
			? ISP_SMART_MAX_VALUE_NUM : component_num;
	...
	for (i=0; i<cfg->component_num; i++) {
		if (SMART_CTRL_FLASH_MAIN != flash_mode || 1 != cfg->component[i].use_flash_val) {
			...
			rtn = _calc_component(&cfg->component[i], bv, bv_gain, ct, &component_result);
		} else {
			rtn = _calc_component_flash(&cfg->component[i], &component_result);
		}
...
static const char * _find_block_name(uint32_t smart_id)
{
	uint32_t i = 0;
	...
	return s_block_name[smart_id];
}

static void _print_debug_file(debug_handle_t debug_file, struct smart_calc_param *calc_param,
				uint32_t mode, struct smart_calc_result *result, char *debug_buf)
{
#if 1
(void)(debug_file); (void)(calc_param); (void)(mode);
(void)(result); (void)(debug_buf);
#else
	...
	rtn = debug_file_open(debug_file, 1, 0);
	if (ISP_SUCCESS != rtn)
		return;

	sprintf(debug_buf, "bv=%d, ct=%d, bv_gain=%d", calc_param->bv,
					calc_param->ct, calc_param->bv_gain);
	debug_file_print(debug_file, debug_buf);
	...
	for (i=0; i<result->counts; i++) {
		...
		block_name = _find_block_name(blk->smart_id);
	...
	debug_file_close(debug_file);
#endif
}

static void _print_smart_result(uint32_t mode, struct smart_calc_result *result)
{
	struct smart_block_result *blk = NULL;
	struct smart_component_result *comp = NULL;
	struct isp_weight_value *weight_value = NULL;
...
smart_handle_t smart_ctl_init(struct smart_init_param *param, void *result)
{
	...
	if (NULL == cxt) {
		ISP_LOGE("malloc0 failed, size: %d\n", sizeof(struct smart_context));
		goto ERROR_EXIT;
	}
	...
	cxt->cur_param = &cxt->tuning_param[cxt->work_mode];
	cxt->debug_file = debug_file_init(DEBUG_FILE_NAME, "wt");
							...
							struct smart_calc_result *result)
{
	...
	ISP_LOGV("SMART_TAG: smart work mode = %d", cxt->work_mode);
	...
	for (i=0; i<smart_param->block_num; i++) {
		...
		rtn = _calc_block(&smart_param->block[i], param->bv, param->bv_gain, param->ct, &cxt->block_result, flash_mode);
	...
	_print_smart_result(cxt->flash_mode, result);
	_print_debug_file(cxt->debug_file, param, cxt->flash_mode, result, (char *)cxt->debug_buf);
...
int32_t smart_ctl_deinit(smart_handle_t handle, void *param, void *result)
{
	...
	if (NULL != cxt->debug_file)
		debug_file_deinit(cxt->debug_file);
...
int32_t smart_ctl_ioctl(smart_handle_t handle, uint32_t cmd, void *param, void *result)
{
	...
	switch (cmd) {
		case ISP_SMART_IOCTL_SET_WORK_MODE:
			rtn = _smart_set_workmode(cxt_ptr, param);
		break;

		case ISP_SMART_IOCTL_SET_FLASH_MODE:
			rtn = _smart_set_flash(cxt_ptr, param);
		break;

		case ISP_SMART_IOCTL_GET_UPDATE_PARAM:
			rtn = _smart_get_update_param(cxt_ptr, param);
		break;
