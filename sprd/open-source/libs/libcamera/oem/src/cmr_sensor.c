...
cmr_int cmr_sensor_get_real_gain(cmr_handle sensor_handle, cmr_uint sensor_id, float *gain)
{
	cmr_int ret = CMR_CAMERA_SUCCESS;
	struct sensor_real_gain_tag *real_gain_param = NULL;
	cmr_uint sensor_param = 0;

	real_gain_param = malloc(sizeof(struct sensor_real_gain_tag));
	if (!real_gain_param) {
		CMR_LOGE("fail to malloc");
		return CMR_CAMERA_NO_MEM;
	}
	real_gain_param->cmd = SENSOR_EXT_REAL_GAIN;
	//gain_thrs_param->param = param_ptr->cmd_value;
	sensor_param = (cmr_uint)real_gain_param;
	ret = cmr_sensor_ioctl(sensor_handle, sensor_id, SENSOR_GET_REAL_GAIN, sensor_param);
	*gain = real_gain_param->gain;
	free(real_gain_param);
	real_gain_param = NULL;
