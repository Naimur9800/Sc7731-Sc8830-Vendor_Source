...
#if defined(CONFIG_CAMERA_SUPPORT_8M)
	s_fd = open(CMR_GRAB_DEV_NAME, O_RDWR, 0);
	p_grab->fd = s_fd;
#else
...
void cmr_grab_setfreq_flag(uint32_t  flag)
{
	cmr_u32		freq_flag = 0;
	cmr_int		ret = 0;
	CMR_LOGI("cmr_grab_setfreq_flag: flag=%d.",flag);
	if (-1 != s_fd)
	{
		freq_flag = flag;
		CMR_LOGI("cmr_grab_setfreq_flag:set freq is %d.",freq_flag);
		ret = ioctl(s_fd, SPRD_IMG_IO_SET_FREQ_FLAG, &freq_flag);
	}
}
#endif
...
cmr_int cmr_grab_path_capability(cmr_handle grab_handle, struct cmr_path_capability *capability)
{
	...
	if (1 == op.parm.capability.path_info[0].support_yuv) {
		capability->is_zoom_post_proc = 1;
	}
#endif
	capability->capture_pause = 1;

	CMR_LOGI("video prev %d scale %d capture_no_trim %d capture_pause %d is_zoom_post_proc %d", capability->is_video_prev_diff,
		capability->hw_scale_available, capability->capture_no_trim, capability->capture_pause, capability->is_zoom_post_proc);
	return ret;
}
