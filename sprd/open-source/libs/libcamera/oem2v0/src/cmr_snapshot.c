...
cmr_int snp_proc_copy_frame(cmr_handle snp_handle, void *data)
{
	...
	if (1 != cxt->req_param.is_video_snapshot && 1 != cxt->req_param.is_zsl_snapshot) {
		...
		if (IMG_DATA_TYPE_RAW == chn_data_ptr->fmt) {
			...
			CMR_LOGI("raw y src_vir = %lx dst_vir = %lx width %d height %d channel_zoom_mode %d",
				src_vir, dst_vir, width, height, proc_param_ptr->channel_zoom_mode);
			cmr_copy((void *)dst_vir, (void *)src_vir, width * height * 3 / 2);
		} else {
			src_vir = (unsigned long)chn_data.yaddr_vir;
			dst_vir = (unsigned long)cxt->req_param.post_proc_setting.chn_out_frm[0].addr_vir.addr_y;
...
cmr_int snp_start_encode(cmr_handle snp_handle, void *data)
{
	...
	if (0 == snp_get_request(snp_handle) || (snp_cxt->cap_cnt == snp_cxt->req_param.total_num)) {
		frame_type.jpeg_param.need_free = 1;
	} else {
		frame_type.jpeg_param.need_free = 0;
	}
	snp_send_msg_notify_thr(snp_handle, SNAPSHOT_FUNC_ENCODE_PICTURE, SNAPSHOT_EVT_START_ENC, (void*)&frame_type, sizeof(frame_type));
	CMR_PRINT_TIME;
	snp_cxt->index = index;
...
cmr_int snp_send_msg_notify_thr(cmr_handle snp_handle, cmr_int func_type, cmr_int evt, void *data, cmr_uint data_len)
{
	...
	if (SNAPSHOT_FUNC_STATE == func_type) {
		sync_flag = CMR_MSG_SYNC_NONE;
				...
				&& ((SNAPSHOT_EXIT_CB_DONE == evt) || (SNAPSHOT_EVT_START_ENC == evt))) {
		sync_flag = CMR_MSG_SYNC_NONE;
	} else {
		switch (evt) {
