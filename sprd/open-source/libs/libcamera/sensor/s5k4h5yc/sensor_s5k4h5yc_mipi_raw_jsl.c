...
static unsigned long _s5k4h5yc_jsl_after_snapshot(unsigned long param);
...
static unsigned long _s5k4h5yc_jsl_flash(unsigned long param);
...
static unsigned long _s5k4h5yc_jsl_set_video_mode(unsigned long param);
...
#define param_update(x1,x2) sprintf(name,"/data/s5k4h5yc_jsl_%s.bin",x1);\
...
static uint32_t Sensor_s5k4h5yc_jsl_InitRawTuneInfo(void)
{
	...
	isp_raw_para_update_from_file(&g_s5k4h5yc_jsl_mipi_raw_info,0);
...
static unsigned long _s5k4h5yc_jsl_write_exposure(unsigned long param)
{
	...
	linetime=s_s5k4h5yc_jsl_Resolution_Trim_Tab[size_index].line_time;
...
static unsigned long _s5k4h5yc_jsl_after_snapshot(unsigned long param)
{
	SENSOR_PRINT("SENSOR_s5k4h5yc_jsl: after_snapshot mode:%ld", param);
...
//uint32_t s_af_step=0x00;
static unsigned long _s5k4h5yc_jsl_write_exposure_ev(unsigned long param)
{
	uint32_t ret_value = SENSOR_SUCCESS;
	uint16_t expsure_line = 0x00;
	uint16_t dummy_line = 0x00;
...
static void _calculate_hdr_exposure(int capture_gain16,int capture_VTS, int capture_shutter)
{
	...
	_s5k4h5yc_jsl_write_exposure_ev(capture_shutter);
}
...
static uint32_t _s5k4h5yc_jsl_SetEV(unsigned long param)
{
	...
	SENSOR_PRINT_HIGH("SENSOR: _s5k4h5yc_jsl_SetEV param: 0x%x s_capture_shutter %d", ext_ptr->param, s_capture_shutter);

	switch(ev) {
	case SENSOR_HDR_EV_LEVE_0:
		_calculate_hdr_exposure(s_s5k4h5yc_jsl_gain,s_capture_VTS,s_capture_shutter/4);
		break;
	case SENSOR_HDR_EV_LEVE_1:
		_calculate_hdr_exposure(s_s5k4h5yc_jsl_gain,s_capture_VTS,s_capture_shutter);
		break;
	case SENSOR_HDR_EV_LEVE_2:
		_calculate_hdr_exposure(s_s5k4h5yc_jsl_gain,s_capture_VTS,s_capture_shutter*2);
		break;
	default:
		break;
	}
...
static uint32_t _s5k4h5yc_jsl_read_otp_gain(uint32_t *param)
{
	...
	*param >>= 8;
	...
	cmd_val[0] = 0x02;
	cmd_val[1] = 0x04;
	rtn = Sensor_ReadI2C(S5K4H5YC_JSL_I2C_ADDR_W,(uint8_t*)&cmd_val[0], cmd_len);
...
LOCAL uint32_t _s5k4h5yc_jsl_erase_otp(unsigned long param)
{
	...
	CMR_LOGI("SENSOR_s5k4h5yc_jsl: _s5k4h5yc_jsl_erase_otp E");
	...
	CMR_LOGI("SENSOR_s5k4h5yc_jsl: _s5k4h5yc_jsl_erase_otp X");
...
LOCAL uint32_t _s5k4h5yc_jsl_read_otp(unsigned long param)
{
	...
	if (NULL != param_ptr->awb.data_ptr && SENSOR_OTP_PARAM_NORMAL == param_ptr->type) {
		...
		param_ptr->awb.size = real_size;
		CMR_LOGI("SENSOR_s5k4h5yc_jsl: awb real_size %d", real_size);
		rtn = lsc_package_convert((void *)(buff + lsc_src_addr), param_ptr->lsc.data_ptr,
			param_ptr->lsc.size, &real_size);
		param_ptr->lsc.size = real_size;
		CMR_LOGI("SENSOR_s5k4h5yc_jsl: lsc real_size %d", real_size);
	}
...
LOCAL uint32_t _s5k4h5yc_jsl_parse_otp(unsigned long param)
{
	uint32_t rtn                  = SENSOR_SUCCESS;
	SENSOR_OTP_PARAM_T* param_ptr = (SENSOR_OTP_PARAM_T*)param;
	uint8_t *buff                 = NULL;
	...
	CMR_LOGI("SENSOR_s5k4h5yc_jsl: _s5k4h5yc_jsl_parse_otp E");
	...
	CMR_LOGI("SENSOR_s5k4h5yc_jsl: _s5k4h5yc_jsl_parse_otp X");
	return rtn;

}

static uint32_t _s5k4h5yc_jsl_get_golden_data(unsigned long param)
{
	uint32_t rtn = SENSOR_SUCCESS;
	SENSOR_OTP_PARAM_T* param_ptr = (SENSOR_OTP_PARAM_T*)param;
	...
	SENSOR_PRINT("SENSOR_s5k4h5yc_jsl: golden: %d", param_ptr->golden.size);
...
static unsigned long _s5k4h5yc_jsl_access_val(unsigned long param)
{
	...
	switch(param_ptr->type)
	{
		...
		case SENSOR_VAL_TYPE_READ_OTP:
			rtn = _s5k4h5yc_jsl_read_otp((unsigned long)param_ptr->pval);
			break;
		case SENSOR_VAL_TYPE_PARSE_OTP:
			rtn = _s5k4h5yc_jsl_parse_otp((unsigned long)param_ptr->pval);
			break;
...
//				*p=&calibration_info;
		...
		case SENSOR_VAL_TYPE_GET_GOLDEN_DATA:
			rtn = _s5k4h5yc_jsl_get_golden_data((unsigned long)param_ptr->pval);
			break;
