...
import com.android.internal.telephony.plugin.InboundSmsHandlerUtils;
...
public class AddonInboundSmsHandlerUtils extends InboundSmsHandlerUtils implements
     AddonManager.InitialCallback {
     private static final String LOGTAG = "AddonInboundSmsHandlerUtils";
	 ...
	 public AddonInboundSmsHandlerUtils(){
        Log.d(LOGTAG, "AddonInboundSmsHandlerUtils enter");
	 }
    ...
    public  boolean plugin_sms(InboundSmsHandler mInboundSmsHandler,InboundSmsTracker tracker,Intent intent){
        String addressNumber = tracker.getAddress();
        if(checkIsBlackNumber(mAddonContext,addressNumber)) {
          SmsMessage[] messages = Intents.getMessagesFromIntent(intent);
          SmsMessage sms = messages[0];
          String body = sms.getMessageBody();
          putToSmsBlackList(mAddonContext,addressNumber,body,(new Date()).getTime());
          mInboundSmsHandler.deleteFromRawTable(tracker.getDeleteWhere(),tracker.getDeleteWhereArgs());
          return false;
        }
        return true;
    }
