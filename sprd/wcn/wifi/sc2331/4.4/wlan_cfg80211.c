...
static void cfg80211_dump_frame_prot_info(int send, int freq,
					  const unsigned char *buf, int len)
{
	...
	buf_idx +=
	    sprintf(print_buf + buf_idx, MACSTR, buf[4], buf[5], buf[6], buf[7],
		    buf[8], buf[9]);
	buf_idx += sprintf(print_buf + buf_idx, "  ");
	buf_idx +=
	    sprintf(print_buf + buf_idx, MACSTR, buf[10], buf[11], buf[12],
